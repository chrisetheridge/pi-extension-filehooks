...
function loadConfig(cwd: string): FileHookRule[] {
  ...
  const configs = [globalConfig, projectConfig].filter(
    (config): config is FileHooksConfig => !!config,
  );
  ...
  return mergedRules.map((rule) => ({
    name: rule.name,
    tools: new Set(
      (rule.tools?.length ? rule.tools : [...DEFAULT_TOOL_NAMES])
        .map((tool) => tool.trim())
        .filter(Boolean),
    ),
...
export default function fileHooksExtension(pi: ExtensionAPI): void {
  pi.on("tool_result", async (event, ctx) => {
    ...
    const matchingRules = rules.filter(
      (rule) => shouldHandle(event, rule) && matchesRule(rule, target),
    );
      ...
      for (const rule of matchingRules) {
        ...
        try {
          ...
          if (result.code !== 0) {
            ...
            throw new Error(
              stderr
                ? `${label} failed for ${target.displayPath}: ${stderr}`
                : `${label} failed for ${target.displayPath}`,
            );
          }
        } catch (error) {
          console.error(error);
